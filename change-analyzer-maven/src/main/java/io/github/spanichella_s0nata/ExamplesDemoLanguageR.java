...
import com.github.gumtreediff.gen.antlr3.r.RTreeGenerator;
 ...
 */
public class ExamplesDemoLanguageR {

  private static String srcFilePathR = "resources/Rscript-v1.r";
  private static String dstFilePathR = "resources/Rscript-v2.r";
  ...
  public static void main(String[] args) {
    ...
    exampleParsingR(srcFilePathR);
    System.out.println("\nEXAMPLE 2: Getting the mappings between two trees:\n");
    exampleMappingR(srcFilePathR, dstFilePathR);
    System.out.println("\nEXAMPLE 3: Computing the actions between two trees:\n");
    exampleActionsR(srcFilePathR, dstFilePathR);
  }

  private static void exampleParsingR(String filePath) {
    try {
      ...
      TreeContext tc2 = new RTreeGenerator().generateFrom().file(filePath);
      Tree t2 = tc2.getRoot(); // retrieves the root of the tree
      // OPTION 2+: can be a one-liner
      Tree t3 = new RTreeGenerator().generateFrom().file(filePath).getRoot();
  ...
  private static void exampleMappingR(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
  ...
  private static void exampleActionsR(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
