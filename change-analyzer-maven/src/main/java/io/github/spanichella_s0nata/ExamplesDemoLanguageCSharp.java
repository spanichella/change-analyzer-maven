 ...
 */
public class ExamplesDemoLanguageCSharp {

  private static String srcFilePathSharp = "resources/cs-class-v1.cs";
  private static String dstFilePathSharp = "resources/cs-class-v2.cs";
  ...
  public static void main(String[] args) {
    ...
    exampleParsingCSharp(srcFilePathSharp);
    System.out.println("\nEXAMPLE 2: Getting the mappings between two trees:\n");
    exampleMappingCSharp(srcFilePathSharp, dstFilePathSharp);
    System.out.println("\nEXAMPLE 3: Computing the actions between two trees:\n");
    exampleActionsCSharp(srcFilePathSharp, dstFilePathSharp);
  }

  private static void exampleParsingCSharp(String filePath) {
    try {
      ...
      TreeContext tc2 = new SrcmlCsTreeGenerator().generateFrom().file(filePath);
      Tree t2 = tc2.getRoot(); // retrieves the root of the tree
      // OPTION 2+: can be a one-liner
      Tree t3 = new SrcmlCsTreeGenerator().generateFrom().file(filePath).getRoot();
  ...
  private static void exampleMappingCSharp(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
  ...
  private static void exampleActionsCSharp(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
