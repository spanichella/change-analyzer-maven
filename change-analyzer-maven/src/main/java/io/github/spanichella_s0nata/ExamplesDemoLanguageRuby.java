 ...
 */
public class ExamplesDemoLanguageRuby {

  private static String srcFilePathRuby = "resources/Ruby-v1.rb";
  private static String dstFilePathRuby = "resources/Ruby-v2.rb";
  ...
  public static void main(String[] args) {
    ...
    exampleParsingRuby(srcFilePathRuby);
    System.out.println("\nEXAMPLE 2: Getting the mappings between two trees:\n");
    exampleMappingRuby(srcFilePathRuby, dstFilePathRuby);
    System.out.println("\nEXAMPLE 3: Computing the actions between two trees:\n");
    exampleActionsRuby(srcFilePathRuby, dstFilePathRuby);
  }

  private static void exampleParsingRuby(String filePath) {
    try {
      ...
      TreeContext tc2 = new RubyTreeGenerator().generateFrom().file(filePath);
      Tree t2 = tc2.getRoot(); // retrieves the root of the tree
      // OPTION 2+: can be a one-liner
      Tree t3 = new RubyTreeGenerator().generateFrom().file(filePath).getRoot();
  ...
  private static void exampleMappingRuby(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
  ...
  private static void exampleActionsRuby(String srcFile, String dstFile) {
    // input data
    Tree src;
    Tree dst;
    try {
